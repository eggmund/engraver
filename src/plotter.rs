...
use pbr::{MultiBar, Units, ProgressBar};
use raw_cpuid::CpuId;
...
impl Plotter {
    ...
    pub fn run(self, mut start_task: PlotterTask, file_num: u64) {
        let cpuid = CpuId::new();
        ...
        if !start_task.quiet && start_task.benchmark {
        ...
        #[cfg(feature = "opencl")]
        let gpu_mem_needed = match &start_task.gpus {
            Some(x) => gpu_get_info(&x, start_task.quiet),
            None => 0,
        };

        #[cfg(feature = "opencl")]
        let gpu_mem_needed = if start_task.zcb {
            gpu_mem_needed
        } else {
            gpu_mem_needed / 2
        };

        let gpu = start_task.gpus.is_some();
        ...
        if start_task.direct_io {
            let sector_size = get_sector_size(&start_task.output_path);
            nonces_per_sector = sector_size / SCOOP_SIZE;
            if start_task.nonces % nonces_per_sector > 0 {
                rounded_nonces_to_sector_size = true;
                start_task.nonces /= nonces_per_sector;
                start_task.nonces *= nonces_per_sector;
            }
        }

        let total_plotsize = start_task.nonces * NONCE_SIZE;
        let nonce_increase = start_task.nonces/file_num;

        if !start_task.quiet {
            println!(
                "RAM: Total={:.2} GiB, Free={:.2} GiB, Target Usage={}",
                memory.total as f64 / 1024.0 / 1024.0,
                get_avail_mem(&memory) as f64 / 1024.0 / 1024.0,
                start_task.mem
            );

            println!("Numeric ID:  {}", start_task.numeric_id);
            println!("Start Nonce: {}", start_task.start_nonce);
            println!("End Nonce: {}", start_task.start_nonce + start_task.nonces);
            println!("File sizes: {:.4} GiB", (nonce_increase * crate::NONCE_SIZE) as f32/(1024f32).powi(3));
            println!("Total plot size to do: {:.4} GiB", total_plotsize as f32/(1024f32).powi(3));
            println!(
                "Nonces:      {}{}",
                start_task.nonces,
        ...
        let mut tasks: Vec<PlotterTask> = vec![start_task; file_num as usize];
        for (i, task) in tasks.iter_mut().enumerate() {
            task.start_nonce += nonce_increase * i as u64;
            task.nonces = nonce_increase;
        }
        tasks.last_mut().unwrap().is_last = true;
        ...
        for task in tasks {
            ...
            let disk_space = free_disk_space(&task.output_path);
            if disk_space < plotsize && !file.exists() && !task.benchmark {
                println!(
                    "Error: insufficient disk space, MiB_required={:.2}, MiB_available={:.2}",
                    plotsize as f64 / 1024.0 / 1024.0,
                    disk_space as f64 / 1024.0 / 1024.0
                );
                println!("Shutting down...");
                return;
            }
